(function () {
  ...
  if (!btnAccept || !btnReject || !btnConfigure || !consentCheckbox) return;
  ...
  function showBanner() {
    ...
    // Foco inicial en el checkbox para guiar al usuario
  ...
  function showConsentWarning(message) {
    if (consentWarning) {
      ...
      // Fallback mínimo si no existe el contenedor
      alert(message);
    }
  }
  ...
  // Mostrar u ocultar en función del consentimiento guardado
  if (!getConsent()) {
    showBanner();
  } else {
    hideBanner();
